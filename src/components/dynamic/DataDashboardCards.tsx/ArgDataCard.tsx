...
import { useGetArgData, useGetAwsData } from "@/hooks/react-query/queries";
...
const ArgDataCard = ({ stationName }: ArgDataCardProps) => {
  const [isOpen, setIsOpen] = useState(false);

  const navigate = useNavigate();
  const { data: stationData, isError, isLoading } = useGetArgData(stationName);

  if (isError || !stationData?.raingaugedata)
    return (
