  ...
  return (
    ...
    >
      <SliderPrimitive.Track className="relative h-2 w-full grow overflow-hidden bg-secondary">
        <SliderPrimitive.Range className="absolute h-full bg-primary dark:bg-[#fbd008]" />
          ...
          {Array.from({ length: notchCount + 1 }).map((_, index) => {
            ...
            if (index % (majorInterval / step) === 0) {
              return (
                <div
                  key={index}
                  className="bg-black h-full w-[2px] dark:bg-[#fbd008]"
                  style={{
      ...
      <SliderPrimitive.Thumb className="block h-5 w-0 border-2 border-primary bg-background transition-colors disabled:pointer-events-none disabled:opacity-50 dark:border-[#fbd008]" />
    </SliderPrimitive.Root>
  );
});
